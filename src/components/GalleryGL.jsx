...
export function GalleryGL({ onSelect, onHoverChange, onExplore, selectedId, hoveredId }) {
  ...
  useEffect(() => {
    ...
    const loadStripeMedia = (s) => {
      ...
      if (isVideo) {
        // Step 1: paint the poster image (~100 KB; AVIF when supported,
        // JPEG fallback otherwise) as the initial texture. This gives every
        ...
        const jpegPoster = project.image.replace(/[^/]+$/, 'poster.jpg');
        const posterPath = avifSupported ? toAvif(jpegPoster) : jpegPoster;
        ...
        video.src = toWebVideo(project.video || project.image);
